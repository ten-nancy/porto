...
class Volume(object):
    ...
    def Check(self):
        return self.conn.CheckVolume(self.path)
...
class Connection(object):
    ...
    def UpdateSpec(self, container, start=False):
        request = rpc_pb2.TContainerRequest()
        if container:
            request.UpdateFromSpec.container.CopyFrom(container)
        request.UpdateFromSpec.start = start
        resp = self.rpc.call(request)
        return Container(self, container.name)
    ...
    def CheckVolume(self, path):
        ...
        return self.rpc.call(request).checkVolume.message

    def ImportLayer(self, layer, tarball, place=None, private_value=None, container=None, timeout=None, verbose_error=False):
        request = rpc_pb2.TContainerRequest()
        ...
        if container:
            request.importLayer.container = container
    ...
    def PullDockerImage(self, name, place=None, auth_token=None, auth_path=None, auth_service=None, platform=None):
        request = rpc_pb2.TContainerRequest()
        ...
        if platform is not None:
            request.pullDockerImage.platform = platform
