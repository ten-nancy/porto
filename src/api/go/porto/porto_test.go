...
func TestCreateFromSpec(t *testing.T) {
	...
	FailOnError(t, c, c.CreateFromSpec(spec, nil, true))
	ExpectProperty(t, c, name, "state", "meta")

	FailOnError(t, c, c.Destroy(name))
}
...
func TestUpdateFromSpec(t *testing.T) {
	...
	FailOnError(t, c, c.CreateFromSpec(spec, nil, false))
	ExpectProperty(t, c, name, "state", "stopped")

	spec.Command = &cmd

	FailOnError(t, c, c.UpdateFromSpec(spec, true))
	ExpectProperty(t, c, name, "command", cmd)
	ExpectProperty(t, c, name, "state", "running")
