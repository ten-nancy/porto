...
TError TNlLink::AddIp6Tnl(const std::string &name,
    ...
    rtnl_link_ip6_tnl_set_flags(Link, IP6_TNL_F_USE_ORIG_TCLASS);
...
TError TNlQdisc::CreateFq(const TNl &nl) {
    struct tcmsg tchdr;
    struct nl_msg *msg;
    struct nlattr *u32;
    TError error;
    ...
    if (Limit) {
        ret = nla_put_u32(msg, TCA_FQ_PLIMIT, Limit);
        if (ret < 0)
            goto free_msg;
    }

    if (Quantum) {
        ret = nla_put_u32(msg, TCA_FQ_QUANTUM, Quantum);
        if (ret < 0)
            goto free_msg;
    }

    if (config().network().has_fq_initial_quantum()) {
        ret = nla_put_u32(msg, TCA_FQ_INITIAL_QUANTUM, config().network().fq_initial_quantum());
        if (ret < 0)
            goto free_msg;
    }

    if (config().network().has_fq_flow_limit()) {
        ret = nla_put_u32(msg, TCA_FQ_FLOW_PLIMIT, config().network().fq_flow_limit());
        if (ret < 0)
            goto free_msg;
    }

    if (config().network().has_fq_buckets()) {
        uint32_t l2 = std::log2(config().network().fq_buckets());
        ret = nla_put_u32(msg, TCA_FQ_BUCKETS_LOG, l2);
        if (ret < 0)
            goto free_msg;
    }
...
#if TCA_FQ_MAX >= 12
    if (config().network().has_codel_ce_threshold()) {
        ret = nla_put_u32(msg, TCA_FQ_CE_THRESHOLD, config().network().codel_ce_threshold());
        if (ret < 0)
            goto free_msg;
    }
    ...
    if (config().network().has_fq_horizon()) {
        ret = nla_put_u32(msg, TCA_FQ_HORIZON, config().network().fq_horizon());
        if (ret < 0)
            goto free_msg;
    }

    if (config().network().has_fq_horizon_drop()) {
        ret = nla_put_u8(msg, TCA_FQ_HORIZON_DROP, config().network().fq_horizon_drop());
        if (ret < 0)
            goto free_msg;
    }
#endif
...
TError TNlBpfFilter::Create(const TNl &nl) {
    TError error = OK;
    struct nl_msg *msg;
    ...
    msg = nlmsg_alloc_simple(RTM_NEWTFILTER, NLM_F_EXCL | NLM_F_CREATE);
    if (!msg)
        return TError("Unable to add bpf filter: no memory");
...
TError TNlBpfFilter::Delete(const TNl &nl) {
    TError error = OK;
    struct nl_msg *msg;
    struct tcmsg tchdr;
    int ret;
    ...
    msg = nlmsg_alloc_simple(RTM_DELTFILTER, 0);
    if (!msg)
        return TError("Unable to del bpf filter: no memory");
