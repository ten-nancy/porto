...
static uint64_t NetBpfProgCheckPeriod;
...
constexpr const char *NET_LIMIT_RX_MAP_NAME = "netlimit_rx_map";

constexpr const char *NET_LIMIT_TX_PROG_PATH = "/sys/fs/bpf/netlimit_tx";
constexpr const char *NET_LIMIT_TX_MAP_NAME = "netlimit_tx_map";

static std::shared_ptr<TBpfProgram> NetLimitRxProgram;
static std::shared_ptr<TBpfProgram> NetLimitTxProgram;
static std::mutex NetLimitProgramsMutex;

static inline std::unique_lock<std::mutex> LockNetLimitPrograms() {
    return std::unique_lock<std::mutex>(NetLimitProgramsMutex);
}
...
bool TNetClass::IsDisabledOnHost() {
    return TNetClass::IsDisabled() || !config().network().enable_host_net_classes();
}
...
void TNetwork::InitializeConfig() {
    ...
    NetBpfProgCheckPeriod = config().network().bpf_progs_update_interval_ms();
...
std::string TNetwork::GetDeviceQdisc(const TNetDevice &dev) {
    if (IsHost() && TNetClass::IsDisabledOnHost())
...
TError TNetwork::SetupShaper(TNetDevice &dev) {
    ...
    error = TrySetupShaperBpf(dev, rate);
    if (error && error.Error != EError::NotFound) {
        L_ERR("Cannot create bpf-based shaper: {}", error);
        return error;
    }

    if (rate && error.Error == EError::NotFound) {
...
TError TNetwork::TrySetupShaperBpf(TNetDevice &dev, uint32_t rate) {
    TError error;
    TBpfMap map;
    ...
    if (TNetClass::IsDisabled())
        return TError(EError::NotFound, "netcls subsystem is disabled, skip bpf shaper");
    if (!TNetClass::IsDisabledOnHost())
        return TError(EError::NotFound, "netcls used for host classes, skip bpf shaper");

    auto netLimitProgramsLock = LockNetLimitPrograms();

    if (!NetLimitTxProgram) {
        auto program = std::make_shared<TBpfProgram>();
        error = program->Open(NET_LIMIT_TX_PROG_PATH);
        if (error)
            return error;
        NetLimitTxProgram = program;
    }

    error = NetLimitTxProgram->GetMap(NET_LIMIT_TX_MAP_NAME, map);
    if (error)
        return error;

    TNlQdisc qdisc(dev.Index, TC_H_CLSACT, TC_H_MAJ(TC_H_CLSACT));
    qdisc.Kind = "clsact";
    (void)qdisc.Delete(*Nl);

    TNlBpfFilter filter(dev.Index);
    filter.Egress = true;
    filter.DirectAction = true;
    filter.Program = NetLimitTxProgram.get();

    TNetLimitKey key(dev.Index, RootClass->LeafHandle);
    TNetLimitParam param(rate);

    if (rate) {
        error = map.Set(key, param);
        if (error) {
            L_ERR("Cannon set map element: {}", error);
            return error;
        }

        error = qdisc.Create(*Nl);
        if (error && error.Errno != ENODEV && error.Errno != ENOENT) {
            L_ERR("Cannot create egress qdisc: {}", error);
            return error;
        }

        error = filter.Create(*Nl);
        if (error) {
            L_ERR("Cannot create egress bpf filter: {}", error);
            return error;
        }

        RootClass->NetLimitTxProgram = NetLimitTxProgram;
    } else if (RootClass->NetLimitTxProgram) {
        error = map.Del(key);
        if (error) {
            L_ERR("Cannon del map element: {}", error);
            return error;
        }

        RootClass->NetLimitTxProgram.reset();
    }
...
TError TNetwork::SetupPolicer(TNetDevice &dev, std::unique_lock<std::mutex> &statLock) {
    ...
    error = TrySetupPolicerBpf(dev, rate, hostNl);
    if (error && error.Error != EError::NotFound) {
        L_ERR("Cannot create bpf-based policer: {}", error);
        return error;
    }
...
TError TNetwork::TrySetupPolicerBpf(TNetDevice &dev, uint32_t rate, const TNl &hostNl)
{
    TError error;
    TBpfMap map;
    ...
    if (TNetClass::IsDisabled())
        return TError(EError::NotFound, "netcls subsystem is disabled, skip bpf policer");
    if (!TNetClass::IsDisabledOnHost())
        return TError(EError::NotFound, "netcls used for host classes, skip bpf policer");

    auto netLimitProgramsLock = LockNetLimitPrograms();

    if (!NetLimitRxProgram) {
        auto program = std::make_shared<TBpfProgram>();
        error = program->Open(NET_LIMIT_RX_PROG_PATH);
        if (error)
            return error;
        NetLimitRxProgram = program;
    }

    error = NetLimitRxProgram->GetMap(NET_LIMIT_RX_MAP_NAME, map);
    if (error)
        return error;

    TNlQdisc fq(dev.Link, TC_H_ROOT, TC_HANDLE(ROOT_TC_MAJOR, 0));
    fq.Kind = "fq";
    (void)fq.Delete(hostNl);

    TNlQdisc clsact(dev.Link, TC_H_CLSACT, TC_H_MAJ(TC_H_CLSACT));
    clsact.Kind = "clsact";
    (void)clsact.Delete(hostNl);

    TNlBpfFilter filter(dev.Link);
    filter.Egress = true;
    filter.DirectAction = true;
    filter.Program = NetLimitRxProgram.get();

    TNetLimitKey key(dev.Link);
    TNetLimitParam param(rate);
    ...
    if (rate) {
        ...
        error = fq.Create(hostNl);
        if (error && error.Errno != ENODEV && error.Errno != ENOENT) {
            L_ERR("Cannot create egress fq qdisc for host peer: {}", error);
            return error;
        }

        error = clsact.Create(hostNl);
        if (error && error.Errno != ENODEV && error.Errno != ENOENT) {
            L_ERR("Cannot create egress clsact qdisc for host peer: {}", error);
            return error;
        }

        error = filter.Create(hostNl);
        if (error) {
            L_ERR("Cannot create egress bpf filter for host peer: {}", error);
            return error;
        }

        EnabledRxLimit = true;
        RootClass->NetLimitRxProgram = NetLimitRxProgram;
    } else if (RootClass->NetLimitRxProgram) {
        error = map.Del(key);
        if (error) {
            L_ERR("Cannon del map element: {}", error);
            return error;
        ...
        RootClass->NetLimitRxProgram.reset();
    }
...
TError TNetwork::SetupQueue(TNetDevice &dev, bool force) {
    ...
    if (IsHost() && TNetClass::IsDisabledOnHost() && dev.TxQueues == 1)
...
void TNetwork::UpdateProcNetStats(const std::string &basename) {
    ...
    for (auto net : *networks) {
        ...
        for (auto ct : net->NetUsers) {
            ...
            else if (basename == "snmp6")
                error = GetProcNetStats(ct->Task.Pid, net->NetSnmp6, basename);
            if (error) {
                if (errno == ENOENT || errno == ESRCH)
                    continue;
...
void TNetwork::NetWatchdog() {
    ...
    while (HostNetwork) {
        ...
        if (GetCurrentTimeMs() - LastBpfProgCheck >= NetBpfProgCheckPeriod) {
            progsDirty = TNetwork::CheckForBpfProgramsUpdate();
            LastBpfProgCheck = GetCurrentTimeMs();
        }
        auto nets = Networks();
        for (auto &net: *nets) {
...
bool TNetwork::CheckForBpfProgramsUpdate() {
    if (TNetClass::IsDisabled() || !TNetClass::IsDisabledOnHost())
        return false;
    ...
    bool dirtyRx = check(NetLimitRxProgram, NET_LIMIT_RX_PROG_PATH);
    bool dirtyTx = check(NetLimitTxProgram, NET_LIMIT_TX_PROG_PATH);

    return dirtyRx || dirtyTx;
}
