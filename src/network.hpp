...
struct TNetClass {
    ...
    std::shared_ptr<TBpfProgram> NetLimitRxProgram;
    std::shared_ptr<TBpfProgram> NetLimitTxProgram;

    TNetClass *Fold;
    TNetClass *Parent;

    static bool IsDisabled();
    static bool IsDisabledOnHost();
};
...
class TNetwork: public TNonCopyable {
    ...
    static bool CheckForBpfProgramsUpdate();
    TError TrySetupShaperBpf(TNetDevice &dev, uint32_t rate);
    TError TrySetupPolicerBpf(TNetDevice &dev, uint32_t rate, const TNl &hostNl);
