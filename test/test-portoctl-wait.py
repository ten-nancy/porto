...
assert subprocess.check_output([portoctl, 'wait', 'test-a']) == b'test-a\n'

assert subprocess.check_output([portoctl, 'wait', 'test-*']) == b'test-a\n'
...
res = subprocess.check_output([portoctl, 'wait', '-A', '-S', 'dead']).decode()
